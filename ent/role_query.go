...
// QueryGmUsers chains the current query on the "gm_users" edge.
func (_q *RoleQuery) QueryGmUsers() *GMUserQuery {
	query := (&GMUserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withGmUsers:     _q.withGmUsers.Clone(),
...
// WithGmUsers tells the query-builder to eager-load the nodes that are connected to
// the "gm_users" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithGmUsers(opts ...func(*GMUserQuery)) *RoleQuery {
	query := (&GMUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGmUsers = query
	return _q
}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withGmUsers; query != nil {
		if err := _q.loadGmUsers(ctx, query, nodes,
			func(n *Role) { n.Edges.GmUsers = []*GMUser{} },
			func(n *Role, e *GMUser) { n.Edges.GmUsers = append(n.Edges.GmUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadGmUsers(ctx context.Context, query *GMUserQuery, nodes []*Role, init func(*Role), assign func(*Role, *GMUser)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Role)
	nids := make(map[int]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.GmUsersTable)
		s.Join(joinT).On(s.C(gmuser.FieldID), joinT.C(role.GmUsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.GmUsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.GmUsersPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*GMUser](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "gm_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
