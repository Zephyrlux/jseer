...
// HasGmUsers applies the HasEdge predicate on the "gm_users" edge.
func HasGmUsers() predicate.Role {
	return predicate.Role(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasGmUsersWith applies the HasEdge predicate on the "gm_users" edge with a given conditions (other predicates).
func HasGmUsersWith(preds ...predicate.GMUser) predicate.Role {
	return predicate.Role(func(s *sql.Selector) {
		step := newGmUsersStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
