...
func (_c *RoleCreate) AddGmUserIDs(ids ...int) *RoleCreate {
	_c.mutation.AddGmUserIDs(ids...)
	return _c
}

// AddGmUsers adds the "gm_users" edges to the GMUser entity.
func (_c *RoleCreate) AddGmUsers(v ...*GMUser) *RoleCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddGmUserIDs(ids...)
}
...
func (_c *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.GmUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
