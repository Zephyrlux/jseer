...
func (_q *AccountQuery) QueryPlayers() *PlayerQuery {
...
func (_q *AccountQuery) WithPlayers(opts ...func(*PlayerQuery)) *AccountQuery {
	query := (&PlayerClient{config: _q.config}).Query()
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withPlayers; query != nil {
		if err := _q.loadPlayers(ctx, query, nodes,
			func(n *Account) { n.Edges.Players = []*Player{} },
			func(n *Account, e *Player) { n.Edges.Players = append(n.Edges.Players, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*Account, init func(*Account), assign func(*Account, *Player)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(player.FieldAccountID)
	}
	query.Where(predicate.Player(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.PlayersColumn), fks...))
	}))
