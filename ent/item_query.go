...
func (_q *ItemQuery) QueryPlayer() *PlayerQuery {
...
func (_q *ItemQuery) WithPlayer(opts ...func(*PlayerQuery)) *ItemQuery {
	query := (&PlayerClient{config: _q.config}).Query()
...
func (_q *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := _q.withPlayer; query != nil {
		if err := _q.loadPlayer(ctx, query, nodes, nil,
			func(n *Item, e *Player) { n.Edges.Player = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ItemQuery) loadPlayer(ctx context.Context, query *PlayerQuery, nodes []*Item, init func(*Item), assign func(*Item, *Player)) error {
...
func (_q *ItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPlayer != nil {
			_spec.Node.AddColumnOnce(item.FieldPlayerID)
		}
	}
