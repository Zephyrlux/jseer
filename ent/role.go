...
type RoleEdges struct {
	...
	// GmUsers holds the value of the gm_users edge.
	GmUsers []*GMUser `json:"gm_users,omitempty"`
...
// GmUsersOrErr returns the GmUsers value or an error if the edge
// was not loaded in eager-loading.
func (e RoleEdges) GmUsersOrErr() ([]*GMUser, error) {
	if e.loadedTypes[1] {
		return e.GmUsers, nil
	}
	return nil, &NotLoadedError{edge: "gm_users"}
}
...
// QueryGmUsers queries the "gm_users" edge of the Role entity.
func (_m *Role) QueryGmUsers() *GMUserQuery {
	return NewRoleClient(_m.config).QueryGmUsers(_m)
}
